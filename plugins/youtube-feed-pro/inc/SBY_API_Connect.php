...
class SBY_API_Connect
{
	...
	public function __construct( $connected_account_or_url = null, $endpoint = '', $params = array() ) {
	...
	public function set_url( $connected_account, $endpoint_slug, $params = [], $api_key = null ) {
		$settings = (new SBY_Settings([], sby_get_database_settings()))->get_settings();

		if ( $api_key === null && in_array( 'api_key', $settings, true ) ) {
			$api_key = $settings['api_key'];
		}

		$num = ! empty( $params['num'] ) && isset( $api_key ) ? (int)$params['num'] : 50;
		...
		if ( $endpoint_slug === 'tokeninfo' ) {
			...
			$access_credentials = !empty( $api_key ) ? 'key=' . $api_key : 'access_token=' . $connected_account['access_token'];
			...
			$access_credentials = isset( $api_key ) ? 'key=' . $api_key : 'access_token=' . $connected_account['access_token'];
			$next_page = '';
