...
abstract class BaseSettingPage extends ServiceProvider {
	...
	public function register() {
		...
		if ( ( true === $this->has_assets ) && isset( $_GET['page'] ) && str_contains( $_GET['page'],
	...
	protected function get_settings_object() {
		return apply_filters( 'sby_localized_settings', [
			...
			'nonce'               => wp_create_nonce( 'sby_admin_nonce' ),
			...
			'connectionURL'       => SBY_OAUTH_PROCESSOR_URL . admin_url( 'admin.php?page=youtube-feed-settings' ),
			'manageLicense'       => 'https://smashballoon.com/account/downloads/?utm_campaign=youtube-pro&utm_source=settings&utm_medium=manage-license',
			'single_video_settings'  => SBY_CPT::get_sby_cpt_settings(),
