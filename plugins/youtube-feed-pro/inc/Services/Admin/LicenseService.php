...
class LicenseService extends ServiceProvider {
	public function register() {
		...
		add_action('admin_notices', [$this, 'sby_renew_license_notice']);
	...
	public function sby_deactivate_license() {
		...
		if( $sby_license_data->license === 'deactivated' ) {
	...
	public function sby_renew_license_notice() {
		...
		$allowed_screens = array(
			'dashboard',
			'feeds-for-youtube_page_sby-feed-builder',
			'feeds-for-youtube_page_youtube-feed-settings',
			'feeds-for-youtube_page_youtube-feed-about',
			'feeds-for-youtube_page_sby-support',
			'feeds-for-youtube_page_youtube-feed-single-videos'
		);
		...
		$ignored_on_dashboard_page = get_user_meta( $user_id, 'sby_ignore_dashboard_license_notice', true );
		...
		if( empty( $sby_license ) || !isset( $sby_license ) ) {
			return;
		}
		...
		$sby_interval = round( abs( strtotime( $sby_todays_date  . ' -1 day') - strtotime( $sby_license_expires_date ) ) / 86400 );
		//Is license expired?
		...
		if( $sby_interval < 30 && !$sby_license_expired ) {
			$sby_expire_countdown = true;
		} else {
			$sby_expire_countdown = false;
		}

		//Check whether it was purchased after subscriptions were introduced
		if( isset($sby_license_data['payment_id']) && intval($sby_license_data['payment_id']) > 762729 ){
			//Is likely to be renewed on a subscription so don't show countdown
			$sby_expire_countdown = false;
		}

		// If license not expired then return;
		if ( !$sby_license_expired ) {
			return;
		}

		// So, license has expired.
		// Lets display the error notice
		echo '<div class="sby-admin-notices sby-license-expired-notice" id="sby-license-notice">';
	...
	private function get_license_key() {
		return get_option( 'sby_license_key', null );
	}
		...
		$output = '<span class="sb-notice-icon sb-error-icon">
            ...
            <div class="sb-notice-body">
                <div class="sb-notice-body-content">
                    <h3 class="sb-notice-title">Your YouTube Feeds Pro license key has expired!</h3>
                    <p>You are no longer receiving updates that protect you against upcoming YouTube changes</p>
                </div>
                <div class="license-action-btns">
                    <a href="'. $this->get_renew_url() .'" target="_blank" rel="nofollow noopener" class="sby-license-btn sby-btn-blue sby-notice-btn">Renew License</a>
                    <button class="sby-license-btn sby-btn-grey sby-notice-btn" id="renew-modal-btn">Why Renew?</button>
                    <button class="sby-license-btn sby-btn-grey sby-notice-btn" id="sby-recheck-license-key">
